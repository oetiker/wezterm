...
| Fancy tab bar | Elements rendered through `box_model`; glyph quads are sized `texture.coords.size * glyph.scale`, so a fallback/bitmap glyph with `scale != 1` is rescaled by the GPU (`box_model.rs:889-914`) | Solid-colour and 1:1 glyph quads are correct; any `glyph.scale != 1` quad is cropped rather than scaled (`purecpu.rs:346-347`) | needs-measurement | degraded | Task 3's finding reproduces and is characterised further here, at case scale (3 tabs + 1 active, produced by `SPAWN_TABS=true`). Regenerate: `FANCY=true SPAWN_TABS=true FUZZ=1 ./compare-case.sh chrome "$PWD/corpus/chrome.sh"` (`tools/purecpu-parity/`) — `out/chrome-gl.png` vs `out/chrome-cpu.png`. Whole-frame AE=1359 (fuzz 0) / 583 (fuzz 1%). Tab-strip crop `1000x32+0+0`: AE=692, no fuzz value clears it (matches Task 3: this region has no established noise floor and is graded by explicit shift/no-shift pixel evidence below, not by a fuzz threshold — see "Grading basis" note above the table). **Confirmed by exact-match test**: `out/chrome-gl.png` row y=17, x 28-45 is bit-identical to `out/chrome-cpu.png` the same row shifted one column left (x 27-44): `compare -metric AE <(convert out/chrome-gl.png -crop 18x1+28+17 +repage png:-) <(convert out/chrome-cpu.png -crop 18x1+27+17 +repage png:-) null:` → AE=0. This is the "bash" tab title text. **The shift is not universal within the tab strip** — it is confined to proportional-width letter glyphs, not every glyph class: digit `1` and the `:` separator (crop `14x14+2+10`) are AE=0/PAE=0 unshifted; the active tab's close `x` icon (crop `20x16+60+8`) is AE=0 unshifted; the top-left and top-right rounded corners of the active tab (crops `10x10+0+0`, `10x10+76+0`) are AE=0/PAE=0. But the *inactive* tabs' close `x` icons (e.g. crop `18x16+148+8`, tab 2) show the same 1px-left shift as the title text — a raw pixel dump (`convert out/chrome-gl.png -crop 18x16+148+8 +repage txt:-` vs the same crop of `out/chrome-cpu.png`) shows identical colour triplets offset by one column, e.g. gl `(128,128,128)(114,114,114)(56,56,56)` at columns 5,6,7 appears in cpu at columns 4,5,6. So the defect tracks specific glyph-run/positioning code paths that differ between the active-tab and inactive-tab box_model branches (`fancy_tab_bar.rs:168-186` active vs `:210-238` inactive), not a single global offset and not confined to one tab. Retro tab bar (below) does not reproduce this at all, confirming it is specific to the fancy tab bar's `box_model` glyph-quad positioning, not to text rendering in general. |
| Retro tab bar | Rendered as ordinary cells plus `util_sprites.white_space` / `filled_box`, both allocated in the same atlas (`render/tab_bar.rs:49-50`, `utilsprites.rs:159-162`) | Same quads; solid-colour quads take the `IS_SOLID_COLOR` fill path, glyphs the 1:1 path (`purecpu.rs:300-341`) | needs-measurement | parity | `FANCY=false SPAWN_TABS=true FUZZ=1 ./compare-case.sh chrome-retro "$PWD/corpus/chrome.sh"` (`tools/purecpu-parity/`) — `out/chrome-retro-gl.png` vs `out/chrome-retro-cpu.png`. Whole-frame AE=793 (fuzz 0) / **0 (fuzz 1%)** — the fancy-tab-bar shift defect does not reproduce here (a genuine defect is never absorbed by fuzz; this is a different, unaffected render path, confirmed separately below). Tab-strip crop `1000x32+0+0`: AE=126 raw, PAE=257 (exactly 1 LSB) — ordinary antialiasing noise, same order of magnitude as the plain-text noise floor Task 3 established for the terminal body, and it clears at fuzz 1% (AE=0). The retro tab bar draws tab titles as ordinary monospace terminal cells rather than `box_model` glyph runs with float-computed positions, which is consistent with it not sharing the fancy bar's positioning defect. |
| Window buttons | Built as `box_model` Elements (polys + glyphs) drawn from the same atlas (`render/window_buttons.rs`) | Same; poly sprites are generated at their final pixel size so the 1:1 blit is exact | needs-measurement | parity | Window buttons only render in `box_model`'s fancy-tab-bar path when `window_decorations` includes `INTEGRATED_BUTTONS` (`fancy_tab_bar.rs:314,372`); the harness's `gen-config.sh` did not previously expose this, so it was extended with a `WINDOW_DECORATIONS` env var (default unset, so every existing case is byte-for-byte unchanged) to reach this otherwise-unreachable render path. `FANCY=true SPAWN_TABS=true WINDOW_DECORATIONS="INTEGRATED_BUTTONS\|RESIZE" FUZZ=1 ./compare-case.sh chrome-buttons "$PWD/corpus/chrome.sh"` (`tools/purecpu-parity/`) — `out/chrome-buttons-gl.png` vs `out/chrome-buttons-cpu.png` show minimise/maximise/close icons rendered top-right. Region crop `140x32+860+0` (the button cluster): AE=0, PAE=0 — bit-identical. This case could have shown the same class of positioning defect as the tab-title glyphs (the buttons are also `box_model` polys/glyphs) but did not, so the parity verdict is not a structural inability to detect the defect. |
| Rounded corners | `poly_quad` rasterises the corner poly into the atlas at exactly the requested corner size and marks it `IS_GRAY_SCALE` (`render/mod.rs:290-328`, `box_model.rs:1038-1092`) | `has_color==4.0` branch: `fg` tinted by atlas alpha; source and destination sizes are equal by construction, so the 1:1 blit is exact (`purecpu.rs:406-411`) | needs-measurement | parity | Exercised in every fancy-tab-bar capture above (every fancy tab, active and inactive, has `border_corners` set — `fancy_tab_bar.rs:183-195,229-241`). Top-left and top-right corners of the active tab, `out/chrome-gl.png` vs `out/chrome-cpu.png`, crops `10x10+0+0` and `10x10+76+0`: AE=0, PAE=0 — bit-identical, unaffected by the adjacent title-text shift. |
| Split dividers | `filled_rectangle` → `IS_SOLID_COLOR` quad, texture ignored by the shader (`render/split.rs:32,55`, `render/mod.rs:266-287`, `glyph-frag.glsl:115-118`) | `IS_SOLID_COLOR` fill path, colour-only, size-independent (`purecpu.rs:300-341`) | needs-measurement | parity | `SPAWN_TABS=true`'s `gui-startup` handler (`tools/purecpu-parity/gen-config.sh`) splits the first tab right at 50%; that handler unconditionally activates the last-spawned tab (tab 3) once done, which put the split off-screen for every capture until fixed by adding an explicit `tab:activate()` at the end of the handler to reselect the split tab (see corpus/gen-config.sh diff — this is what "gui-startup does not exercise what the row claims" means in practice: the event fired, but the config it produced could not show a diverider in any capture without this fix). With the fix, `out/chrome-gl.png` vs `out/chrome-cpu.png` show the vertical divider at x=495; column crop `1x661+495+32` (full body height): AE=0, PAE=0 — bit-identical. |
...
One incidental finding while establishing these crops: `compare-case.sh`'s
default body crop (`1000x661+0+32`) reported `PAE = 514` (2 LSB, over Task
3's 257 gate) for the `chrome`/`chrome-retro` cases. This is **not** a chrome
or window-chrome finding — narrowing the crop shows it is 100% contained in
two lines of interactive-shell-prompt text (nerd-font colour icons rendered
by the user's real shell startup, an artifact of `gui-startup` launching a
login shell rather than the scripted corpus) in the pane content, with the
remaining 597 rows of blank terminal body bit-identical (`AE = 0`). It belongs
under "Text glyphs" / general text-rendering noise, not under any Task 5 row,
and is recorded here only so the raw `compare-case.sh` output for these two
cases isn't misread as a chrome defect.
