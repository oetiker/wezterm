...
>
> **Read the two behaviour columns as written, in the past tense: they describe
> the PRE-FIX binary.** Only the Verdict and Evidence cells were rewritten to
> the post-fix binary. The `GPU behaviour` and `PureCpu behaviour` cells are
> left as the review wrote them, for the same reason `findings.md` leaves its
> finding texts alone — they are the record of what was found, and they explain
> what each fix had to do — but they are phrased in the present tense and so
> now assert as current a set of defects the same row's Evidence says are
> fixed. **Where a behaviour cell and an Evidence cell disagree, the Evidence
> cell is the current one.** The sharpest instance is the subpixel-AA row,
> whose behaviour cell describes a single scalar alpha falling back to
> grayscale antialiasing while its Evidence records `fd6142a` implementing
> per-channel subpixel AA; the cursor-blink, sixel, iTerm2, animated-GIF and
> DECDWL rows read the same way. **Line citations in those cells have drifted
> too** and are not maintained: `purecpu.rs:344-360` and `:346-347` no longer
> point at the blit — `call_draw_purecpu` now begins at `purecpu.rs:348` — so
