...
Counting what is actually behind the verdicts: **14 rows carry a command and
numbers, 2 rest on code citations alone** (scaled fallback/bitmap glyphs,
subpixel antialiased text), **5 are out-of-scope**.
...
| Double-width / double-height lines (DECDWL/DECDHL) | Glyph destination is scaled by `width_scale` / `height_scale` while the atlas source stays at base size; the GPU rescales (`render/screen_line.rs:50-63`, `632-647`) | 1:1 blit clips to the smaller of source and destination, so the glyph is drawn at base size in the top-left of the enlarged cell (`purecpu.rs:346-347`). The DECDHL **bottom** half is worse than that: `render_screen_line` early-returns for a double-height-bottom line because the *top* line's quad is drawn at twice the cell height and is meant to cover both row bands (`screen_line.rs:31-38`) — the 1:1 blit clips that quad to the source height, so the bottom band receives no quad at all and stays empty (`purecpu.rs:346-347`) | needs-measurement | degraded | **Originally classified `by-reading`; measured in a Task 8 addendum, and the prediction is confirmed, with numbers.** (Method was moved to `needs-measurement` when the measurement landed, so this row's Method matches its Evidence like every other measured row.) (This is the first by-reading prediction in this review that measurement *confirmed*; the two before it, inline images and cursor blink, were both overturned on their preconditions, so the untested precondition here was "does `\033#6` actually reach this path in this fork?".) `FUZZ=1 ./compare-case.sh dwl "$PWD/corpus/dwl.sh"` — `out/dwl-gl.png` vs `out/dwl-cpu.png`. **This case prints `compare-case.sh`'s Gray-colourspace warning for both captures, and it is expected here, not a problem** (documented at the point of use, as `compare-case.sh`'s own comment asks and as the Fancy tab bar and Cursor static rows already do): `corpus/dwl.sh` is deliberately monochrome text with no colour content, so Gray is the correct colourspace, not the "backend hadn't drawn anything yet" case the warning exists to catch. Confirmed by inspecting both PNGs directly — each shows the corpus's five lines — and by the control lines measuring `PAE=257` rather than the 0 a genuinely blank pair would give. Ignore the two warnings on this case; do not ignore them on any case not documented this way. Whole-frame AE=10476 (fuzz 0) / 9349 (fuzz 1%); **body PAE=45232 (176 LSB, fails the `PAE<=257` gate by two orders of magnitude)**. Numbers reproduced identically on a second run from a clean shell. **The corpus carries its own reachability control, and it is what makes the result readable:** `corpus/dwl.sh` prints the same class of text as an explicit single-width line (`\033#5`) *and* as a double line, in one capture, so a null result cannot be mistaken for parity — had the escape never reached `Line::set_double_width`, all five lines would have come back at the noise floor. They did not. Per-line crops (row pitch 22px, body starts y=32): control single-width lines **`1000x22+0+32` PAE=257 and `1000x22+0+120` PAE=257** — exactly the Task 3 noise floor, i.e. parity in the same frame that fails elsewhere; DECDWL `1000x22+0+54` **PAE=45232**; DECDHL top `1000x22+0+76` **PAE=45232**; DECDHL bottom `1000x22+0+98` **PAE=45232**. Ink-pixel counts (threshold 30%) quantify what is actually lost: control lines gl/cpu = 1152/1149 and 975/970 (**ratio 1.00** — identical), DECDWL = 2284/1139 (**ratio 2.01** — OpenGL lays down exactly twice the ink, i.e. it stretches each glyph to the doubled cell while PureCpu draws it at base width and leaves the rest of the cell empty), DECDHL top = 1688/804 (**2.10**), DECDHL bottom = 2008/**119** (**ratio 16.9**). The bottom-half row is not drawn wrongly, it is **not drawn**: over the scan rows that actually carry the bottom half's ink, **y=99-110, OpenGL lays down 1824 ink px and PureCpu exactly 0** (per-row, at threshold 30%; the residual 119 px in the band total sit on y=98, the band boundary). The range is quoted as y=99-110 rather than to the end of the 22px band because OpenGL itself has zero ink from y=111 onward — rows past 110 would pad the claim with pixels where neither backend draws anything. Visually: OpenGL renders DECDWL as wide text and DECDHL as one double-height line spanning two rows; PureCpu renders base-size glyphs spaced at the doubled cell pitch (letters small, with gaps) and drops the DECDHL bottom half entirely. Both backends' logs are clean of `Configuration Error`, and they are positively discriminated (the `-gl` log carries libEGL/DRI lines, the `-cpu` log is 0 bytes), so this is a genuine cross-backend difference and not a config fallback. Verdict stays `degraded` and not `missing` under the named-feature rule: the text of a double line is delivered and legible, only its geometry is wrong — the DECDHL bottom half is `missing` as a scoped sub-case. **Why a blank band does not make the whole row `missing`:** `screen_line.rs:32` requires the top and bottom lines of a DECDHL pair to hold **the same content** — the pair is one logical line the terminal renders across two rows. So the blank bottom band withholds nothing the top band is not already showing; the text is all there, once, at the wrong size. That is what separates this from I1/I2 in `findings.md`, where a blank region means real output was genuinely lost and never appears anywhere |
| Scaled fallback / bitmap glyphs (`glyph.scale != 1`) | Destination is `sprite size * glyph.scale`; the GPU rescales (`render/screen_line.rs:380-393`, `termwindow/box_model.rs:902-913`). `scale` is set below 1 for oversized or unscaled bitmap glyphs such as colour-emoji fonts (`glyphcache.rs:749-798`) | 1:1 blit crops the sprite to the destination rectangle instead of downscaling | by-reading | degraded | `glyphcache.rs:749-798`, `termwindow/box_model.rs:902-913` vs `purecpu.rs:346-347`. **Not measured, deliberately (review round 1, §3).** This is the same `purecpu.rs:346-347` source/destination mismatch the Double-width/height row above now demonstrates with numbers, so the mechanism is no longer resting on reading alone. What is font-dependent is the *precondition*: some installed font must actually yield `glyph.scale != 1`. A capture that happened to use no such font would return a null result, and a null result here would look exactly like `parity` — which is the Task 4 CRIT-1 mistake this review has already made once and must not repeat. Measuring it would first require establishing which installed font produces a scaled sprite; that was not done, so the row rests on the shared mechanism rather than on a number that could be an artefact |
| Subpixel antialiased text (`freetype_render_target = "HorizontalLcd"`) | Layer 1 is drawn with dual-source blending using the per-channel `colorMask` output (`render/draw.rs:172-193`, `244`, `260-266`; `glyph-frag.glsl:14-17`, `147-152`) | The rasteriser has a single scalar alpha and no per-channel mask; `blend_over` blends one alpha for all channels (`purecpu.rs:529-543`). **PureCpu does not lose the glyph: it falls back to ordinary grayscale antialiasing**, because the LCD path already stores a usable scalar alpha alongside the per-channel coverage | by-reading | degraded | `render/draw.rs:181-193` vs `purecpu.rs:529-543`. **What the user actually sees (review round 1, Important 3): grayscale-antialiased text, not broken text.** The path is traceable end to end by reading, with no runtime-dependent precondition — `freetype_render_target = "HorizontalLcd"` does set `use_lcd_subpixel` in this fork (the skrifa stack honours it; it is not dead config), and `subpixel_mask_to_rgba` writes per-channel coverage into RGB *and* `rgba[dst+3] = linear_alpha` where `linear_alpha = r.max(g).max(b)` (`skrifa_rasterizer.rs:687,701`), with `has_color = false` (`:252-255`). A `has_color==0.0` sprite takes PureCpu's `IS_GLYPH` branch, which sets `out_a = tex_a` and `out_r/g/b = fg` (`purecpu.rs:412-419`). So the glyph is entirely present, correctly positioned and legible, driven by a max-of-channels coverage alpha — slightly heavier than a true grayscale mask. **What is lost is the subpixel horizontal resolution and the colour fringing, not the text.** `degraded` is the right token, and it is the best-founded of the rows resting on code alone: unlike the two by-reading predictions this review overturned, this one has no precondition left to fail — if the user sets `HorizontalLcd`, the divergence follows unconditionally. Not measured: doing so would need a config knob the harness does not expose |
