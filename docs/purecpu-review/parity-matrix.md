...
| Inline image: sixel | Image is decoded into the same glyph atlas; each covered cell gets a quad whose texture rect is the cell's fraction of the sprite and whose destination is exactly one cell — the GPU rescales source to destination (`render/mod.rs:468-518`) | Textured quads are blitted 1:1: `blit_w = tex_w.min(dest_w)`, `blit_h = tex_h.min(dest_h)`, source stepped in lockstep with destination. No scaling exists in the rasteriser, so whenever the image's per-cell source region is not exactly `cell_width x cell_height` the image is cropped (or leaves gaps), never resampled (`purecpu.rs:344-360`) | needs-measurement | parity (native size only — see report) | Regenerate: `FUZZ=1 ./compare-case.sh images "$PWD/corpus/images.sh"`. Native 64x64 sixel: `out/images-gl.png` vs `out/images-cpu.png`, region crop `64x64+0+55`: AE=0, PAE=0 (bit-identical). Sixel carries no display-size escape parameter of its own, so its only theoretical route to the crop defect is `AllowImage::Scale` atlas downscale (`paint.rs:72-76`) firing when the atlas fails to grow — attempted with a 300x300 sixel (`out/images-gl.png`/`out/images-cpu.png`, region crop `300x300+0+341`): still parity (AE=3300/PAE=1542, both far below the degraded iTerm2 case's AE=39360/PAE=61423; no "texture space"/`AllowImage` log line in either backend's log). Could not reproduce the atlas-downscale route directly: this machine's `GL_MAX_TEXTURE_SIZE` is 16384 (`glxinfo -l`), and reaching that requires a source image far too large to be practical in this harness — recorded as unmeasured rather than inferred from the iTerm2 row below. See Task 4 fix-round-1 report |
| Inline image: iTerm2 OSC 1337 | Same path as sixel (`populate_image_quad`, `render/mod.rs:441-521`); `AllowImage::Scale(n)` may store a downscaled sprite in the atlas, widening the source/destination ratio | Same 1:1 crop as sixel; a downscaled sprite makes the mismatch larger | needs-measurement | degraded | Regenerate: `FUZZ=1 ./compare-case.sh images "$PWD/corpus/images.sh"`. Native 64x64: `out/images-gl.png` vs `out/images-cpu.png`, region crop `64x64+0+143`: AE=0, PAE=0 (bit-identical) — parity only holds when the requested display size equals the sprite's native size. Requested at non-native size (200x132px, and again at 20x4 cells) in the same corpus: region crop `200x200+0+77` (merged view of both non-native blocks after terminal scroll): AE=39360, PAE=61423 (0.937 — near-maximal), `cpu` ink stddev 7512.89 vs `gl` ink stddev 12814.6 (about 59%, confirming PureCpu draws markedly less ink, not just displaced ink). Visual: OpenGL draws a solid stretched gradient block; PureCpu draws a dotted grid of tiny cropped fragments, one small top-left tile per covered cell, background showing through the rest of every cell — exactly the `purecpu.rs:346-347` crop-instead-of-scale defect at full severity, reachable from an ordinary escape sequence. See Task 4 fix-round-1 report |
