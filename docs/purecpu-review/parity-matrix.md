...
One row runs the other way: `Double-width / double-height lines` was classified
`by-reading` at the outset and then **measured anyway**, in a Task 8 addendum,
because its reading rested on an untested precondition. Its Method cell still
reads `by-reading` — that is what it was classified as — and its Evidence cell
carries the measurement. **Where Method and Evidence disagree about what was
done, Evidence is authoritative.** Counting what is actually behind the
verdicts: **14 rows carry a measurement, 2 rest on code citations alone**
(scaled fallback/bitmap glyphs, subpixel antialiased text), **5 are
out-of-scope**.
