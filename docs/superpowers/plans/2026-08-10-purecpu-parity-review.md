...
- **Grade a row against its own named feature, not against the mechanism behind it.** Two rows can share one root cause and still earn different verdicts: "Inline image: animated GIF" is an image that draws correctly but does not animate, so the feature is partly delivered (`degraded`); "Cursor blink" *is* the blink, and none of it happens, so nothing is delivered (`missing`). Both were graded correctly in different tasks by different implementers, but the rule was written down nowhere — a consolidator reading the two side by side would reasonably think one is wrong. Controller ruling (2026-08-10, Task 6 re-review N6). When a row's verdict is scoped to one path (e.g. config-driven vs app-driven), the Verdict cell holds the token for the named feature's common case and the Evidence states the scope.
