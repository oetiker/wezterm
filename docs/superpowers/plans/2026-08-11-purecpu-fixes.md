...
- **Cell backgrounds are seam-free only while the bg rect is built from `cell_width`, and `experimental_pixel_positioning` already takes it off that path.** Answered by Task 15 and corrected by its review; not closed. `screen_line.rs:231-242` builds one cluster's right edge as `(left + x_pos) + width` and the next cluster's left as `left + (x_pos + width)`; those are different floats whenever the terms are fractional. On the default path they are not: `cell_width = cell_size.width as f32 * width_scale` is an integer by enumeration of all six producers (`cell_size` is a `Size2D<isize>`; `width_scale` is only ever the literal 1.0 or 2.0), so `i * cw` is exact and the two forms are bit-identical over 79.3M measured boundaries — 0 differing edge pairs, 0 coverage disagreements. Under `experimental_pixel_positioning` (`config.rs:796`) the rect comes from `item.x_pos` / `item.pixel_width`, sums of fractional `glyph.x_advance` values, and ~2.3% of boundaries then carry non-identical edges with roughly 1 in 640,000 landing on a pixel centre and seaming or overlapping (modelled, not instrumented — the mechanism is read from the source, only the rate is a model). Fractional DPI scaling would put the default path there too. Pinned by `adjacent_solid_quads_can_seam_at_a_fractional_cell_width`. **Not a PureCpu parity defect:** GL rasterises the same two edges under the same pixel-centre rule and drops the same column, so this is a pre-existing wezterm artifact, today behind an experimental flag.
